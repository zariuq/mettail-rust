...
ARTIFACT_DIR="$ROOT/.artifacts/tinymlsmoke_roundtrip"
...
echo "[1/4] Exporting TinyML smoke artifacts from Lean"
(
  ulimit -v 6291456
  cd "$METTAPEDIA_DIR"
...
cat > "$ROOT/languages/src/tinyml_from_lean.rs" <<'RS'
#![allow(
...
RS
cat "$LANG_TXT" >> "$ROOT/languages/src/tinyml_from_lean.rs"
printf '\n' >> "$ROOT/languages/src/tinyml_from_lean.rs"

echo "[3/4] Building round-trip checker"
(
  ulimit -v 6291456
  cd "$ROOT"
  cargo build -q -p mettail-languages --bin tinymlsmoke_roundtrip
)
...
(
  ulimit -v 6291456
  cd "$ROOT"
  cargo run -q -p mettail-languages --bin tinymlsmoke_roundtrip -- \
    "$INPUT_TERM" "$EXPECTED_TERM"
)

echo "TinyML smoke round-trip passed."
