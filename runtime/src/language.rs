...
/// Function signature for a registered native MORK backend runner.
pub type MorkBackendRunnerFn = fn(&dyn Term) -> Result<AscentResults, String>;

#[derive(Clone, Copy)]
struct MorkBackendRegistration {
    run: MorkBackendRunnerFn,
    use_for_auto: bool,
}

fn mork_backend_registry() -> &'static Mutex<HashMap<String, MorkBackendRegistration>> {
    static REGISTRY: OnceLock<Mutex<HashMap<String, MorkBackendRegistration>>> = OnceLock::new();
    REGISTRY.get_or_init(|| Mutex::new(HashMap::new()))
}

fn lookup_registered_mork_backend(
    language_name: &str,
) -> Result<Option<MorkBackendRegistration>, String> {
    let guard = mork_backend_registry()
        .lock()
        .map_err(|_| "MORK backend registry lock poisoned".to_string())?;
...
/// to opt into artifact-driven MORK execution without REPL-level ad-hoc routing.
pub fn register_mork_backend_runner(
    language_name: &str,
    runner: MorkBackendRunnerFn,
    use_for_auto: bool,
) -> Result<(), String> {
    if language_name.trim().is_empty() {
        return Err("cannot register MORK backend runner for empty language name".to_string());
    }
    let mut guard = mork_backend_registry()
        .lock()
        .map_err(|_| "MORK backend registry lock poisoned".to_string())?;
    guard.insert(language_name.to_string(), MorkBackendRegistration { run: runner, use_for_auto });
    Ok(())
}
...
pub trait Language: Send + Sync {
    ...
    fn supports_backend(&self, backend: RuntimeBackend) -> bool {
        match backend {
            RuntimeBackend::Auto | RuntimeBackend::Ascent => true,
            RuntimeBackend::Mork => lookup_registered_mork_backend(self.name())
                .ok()
                .flatten()
                .is_some(),
        }
    ...
    ///
    /// Default implementation routes `Auto`/`Ascent` to `run_ascent`, unless
    /// a registered native MORK adapter requests `Auto` routing.
    fn run_backend(
        &self,
        term: &dyn Term,
        ...
        match backend {
            RuntimeBackend::Auto => {
                if let Some(reg) = lookup_registered_mork_backend(self.name())? {
                    if reg.use_for_auto {
                        return (reg.run)(term);
                    }
                }
